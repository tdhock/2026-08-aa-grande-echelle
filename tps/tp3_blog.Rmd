...
```{r}
library(data.table)
smallfash <- fread("MNIST_FashionMNIST_small.csv")
...
```{r}
smallfash[, label := factor(y)]
task.list <- list()
for(dname in unique(smallfash$data.name)){
  mtask <- mlr3::TaskClassif$new(
    dname, smallfash[data.name==dname], target="label")
  mtask$col_roles$stratum <- "label"
  mtask$col_roles$feature <- grep("^[0-9]+$", names(smallfash), value=TRUE)
  task.list[[dname]] <- mtask
}
task.list
...
```{r}
n.pixels <- 28 # width et height de chaque image
learner.list <- list(
